...
const SignUp = ({ switchToLogin }) => {
  ...
  const dosignup = async () => {
    try {
      const response = await fetch("http://localhost:3000/api/DeliveryPersonsApi/Signup", {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
        body: JSON.stringify({
          action: "signup", Name, Email, Age, Phone_No, Password,Address
        }),
      });
      const result = await response.json();
      //Apne aap he solve ho jaate Erros pta nhi kese
      if (result && result.success) {
        ...
        // delete result.Age;
        // delete result.City;
        // localStorage.setItem("UsersSignup", JSON.stringify(result));
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSignUp}>
        ...
        {/* Age Field */}
        <div className="mb-4">
          <label className="block text-gray-400 text-sm font-bold mb-2" htmlFor="age">
            Age
          </label>
          <input
            className="w-full px-3 py-2 border border-gray-300 rounded-md"
