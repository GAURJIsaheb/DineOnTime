...
const Login = () => {
  ...
  const gotoUserSidePage = () => {
    router.push('/EnterPageforUsers');
  };
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch("/api/usersAccountApi/LoginApi", {
        method: "POST",
      ...
      if (res.success) {
        ...
        localStorage.setItem("LoginData", JSON.stringify(res.userData)); // Store user data excluding password
        gotoUserSidePage(); // Go to Dashboard
      } else {
        setEmail("");
        setPassword("");
  ...
  return (
    <div >
      <h2 className="text-3xl font-bold text-center mb-6 text-white">Food Delivery App</h2>
