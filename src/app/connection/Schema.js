...
const DatabaseStructure = new mongoose.Schema({
  Name: {
    type: String,
    required: true
  },
...
DatabaseStructure.pre('save', async function(next) {
  const user = this;
...
DatabaseStructure.methods.comparePassword = async function(inputPassword) {
  return await argon2.verify(this.Password, inputPassword);
};

export const SchemaVariable = mongoose.models.project
  || mongoose.model("project", DatabaseStructure);
